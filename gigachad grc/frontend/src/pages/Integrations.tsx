...
export default function Integrations() {
  ...
  useEffect(() => {
    ...
    const integrationType = (searchParams.get('integrationType') || '').toLowerCase();
    const redirectTarget = (searchParams.get('redirect') || '').toLowerCase();

    if (!autoSync || autoSyncTriggered || !createdIntegrations.length) {
      return;
    }

    if (integrationType && integrationType !== 'cloudguard') {
      return;
    }

    const cloudguardIntegration = createdIntegrations.find(
      (integration) => String(integration?.type || '').toLowerCase() === 'cloudguard'
    );

    if (!cloudguardIntegration) {
      setAutoSyncTriggered(true);
      toast.error('CloudGuard integration is not configured in Gigachad GRC yet.');
      return;
    }

    if (cloudguardIntegration.status !== 'active') {
      setAutoSyncTriggered(true);
      toast.error('CloudGuard integration exists, but it is not active yet.');
      return;
    }
    ...
    (async () => {
      try {
        await integrationsApi.triggerSync(cloudguardIntegration.id);
        await queryClient.invalidateQueries({ queryKey: ['integrations'] });
        await queryClient.invalidateQueries({ queryKey: ['integrations-stats'] });
        toast.success('CloudGuard sync started');

        if (redirectTarget === 'risks') {
          navigate('/risks?source=cloudguard');
        }
      } catch (error) {
        toast.error('Failed to trigger CloudGuard sync');
      } finally {
        const nextParams = new URLSearchParams(searchParams);
