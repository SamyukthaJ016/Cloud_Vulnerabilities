...
class CredentialManager:
    ...
    def _build_generated_credential_name(self, credential: CloudCredential) -> str:
        """Generate a readable credential name when the client leaves it blank."""
        timestamp = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
        provider = (credential.cloud_provider or "credential").strip().lower()

        if provider == "aws":
            descriptor = normalize_aws_region(credential.aws_region)
        elif provider == "gcp":
            descriptor = (credential.gcp_project_id or "project").strip()
        elif provider == "kubernetes":
            descriptor = (
                credential.kubernetes_cluster_name
            ...
            descriptor = (credential.azure_subscription_id or "subscription").strip()
        else:
            descriptor = "credential"

        normalized_descriptor = "-".join(
            chunk for chunk in descriptor.replace("/", "-").replace(" ", "-").split("-") if chunk
        ) or "credential"
        return f"{provider}-{normalized_descriptor}-{timestamp}"

    def save_credential(self, credential: CloudCredential) -> Tuple[int, str]:
        """
        Save encrypted credential to database
        Returns credential ID and whether the row was created or updated.
        """
        conn = self._get_connection()
        cur = conn.cursor()
        ...
        # Ensure user exists
            ...
            # New or updated credentials should remain pending until validation completes.
            credential.is_valid = False

            if credential.cloud_provider == "kubernetes" and credential.kubernetes_kubeconfig:
                credential.kubernetes_kubeconfig = prepare_kubeconfig_text(
        ...
        # Now log the audit AFTER the credential is saved
            ...
            return credential_id, save_action
...
# Replace the existing get_credentials method with this:
    ...
    def get_credentials(self, credential_id: int, user_id: str) -> Optional[CloudCredential]:
        ...
        # 🔥 DECRYPT sensitive fields
            ...
            if credential_dict.get("cloud_provider") == "kubernetes" and credential_dict.get("kubernetes_kubeconfig"):
                try:
                    credential_dict["kubernetes_kubeconfig"] = prepare_kubeconfig_text(
                        credential_dict["kubernetes_kubeconfig"]
                    )
                except KubeconfigPreparationError as exc:
        ...
        try:
            if provider:
                cur.execute(
                    ...
                    ORDER BY is_default DESC, updated_at DESC NULLS LAST, last_used DESC NULLS LAST
                    """,
                    (user_id, provider)
                )
            else:
                cur.execute(
                    ...
                    FROM cloud_credentials 
                    WHERE user_id = %s
                    ORDER BY cloud_provider, is_default DESC, updated_at DESC NULLS LAST, last_used DESC NULLS LAST
                    """,
                    (user_id,)
                )
    ...
    def _validate_aws_credential(self, credential: CloudCredential) -> Dict[str, Any]:
        ...
        try:
            ...
            session = boto3.Session(
                ...
                region_name=normalize_aws_region(credential.aws_region)
            )
    ...
    def _validate_kubernetes_credential(self, credential: CloudCredential) -> Dict[str, Any]:
        """Validate Kubernetes credentials using kubeconfig."""
        temp_path = None
        try:
            ...
            if not credential.kubernetes_kubeconfig:
                return {
                    'valid': False,
                    'message': 'No Kubernetes kubeconfig provided',
                    'details': {}
                }
            ...
            api_client = config.new_client_from_config(
                ...
                context=credential.kubernetes_context or None,
            )

            version_api = client.VersionApi(api_client)
            version = version_api.get_code()
            contexts, active_context = config.list_kube_config_contexts(config_file=temp_path)
            context_name = credential.kubernetes_context or (active_context or {}).get("name")
            cluster_name = credential.kubernetes_cluster_name
            if not cluster_name and active_context:
                cluster_name = (active_context.get("context") or {}).get("cluster")

            return {
                'valid': True,
                'message': 'Kubernetes credentials validated successfully',
                'details': {
                    'context': context_name,
                    'cluster_name': cluster_name,
                    'server_version': getattr(version, 'git_version', None)
                }
            }
            ...
            return {
                'valid': False,
                'message': f'Kubernetes validation error: {str(e)}',
                'details': {}
            }
        finally:
        ...
        try:
            cur.execute(
                ...
                (session_id, user_id, aws_credential_id, gcp_credential_id, 
                 openai_credential_id, azure_credential_id, kubernetes_credential_id,
                 scan_config, expires_at)
