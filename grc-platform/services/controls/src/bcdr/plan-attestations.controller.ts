...
import { JwtAuthGuard } from '@gigachad-grc/shared';
...
export class PlanAttestationsController {
  ...
  @ApiResponse({ status: 404, description: 'Attestation not found' })
  async getAttestation(@Param('id') id: string, @Req() req: BCDRRequest) {
    return this.attestationsService.findOne(id, req.user.organizationId);
  }
  ...
  async listAttestations(
    ...
    return this.attestationsService.findAll(req.user.organizationId, filters);
  }
  ...
  async getAttestationStats(@Req() req: BCDRRequest) {
    return this.attestationsService.getStats(req.user.organizationId);
  }
}
