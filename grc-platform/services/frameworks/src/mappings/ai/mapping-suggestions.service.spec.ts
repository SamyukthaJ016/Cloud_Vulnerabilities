...
describe('MappingSuggestionsService', () => {
  ...
  const AUTHORIZATION = 'Bearer tenant-bound-test-token';
  const FW_ID = 'fw-1';
  const REQ_ID = 'req-1';
  const CTL_ID = 'ctl-1';
  ...
  beforeEach(async () => {
    ...
    const suggestWithAuthorization = service.suggest.bind(service);
    jest.spyOn(service, 'suggest').mockImplementation((dto, userId, organizationId, authorization) =>
      suggestWithAuthorization(dto, userId, organizationId, authorization || AUTHORIZATION)
    );
  });
