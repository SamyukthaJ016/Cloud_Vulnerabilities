...
import { normalizeCloudGuardEvidence } from './tools/cloudguard-evidence.js';
...
const tools: Tool[] = [
  ...
  {
    name: 'normalize_cloudguard_evidence',
    description:
      'Normalize CloudGuard or scanner JSON into GRC evidence mapped to CERT-In MSME control families',
    inputSchema: {
      type: 'object',
      properties: {
        sourceName: {
          type: 'string',
          description: 'Source name, for example CloudGuard, Local MCP Scanner, or Manual Evidence',
        },
        scanId: {
          type: 'string',
          description: 'CloudGuard scan ID or external scanner run ID',
        },
        target: {
          type: 'string',
          description: 'Cloud account, project, domain, cluster, repo, or uploaded file target',
        },
        findings: {
          type: 'array',
          description: 'Array of scanner findings to normalize',
          items: { type: 'object' },
        },
        raw: {
          type: 'object',
          description: 'Raw scanner JSON with findings, vulnerabilities, or results arrays',
        },
      },
    },
  },
  ...
  try {
    ...
    switch (name) {
      ...
      case 'scan_vulnerability':
        ...
        break;
      case 'normalize_cloudguard_evidence':
        result = await normalizeCloudGuardEvidence(toolArgs as Parameters<typeof normalizeCloudGuardEvidence>[0]);
        break;
